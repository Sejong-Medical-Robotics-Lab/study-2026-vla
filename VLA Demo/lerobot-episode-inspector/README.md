...
`J`/`K`는 INSTRUCTION 줄의 **‹ 이전 / 다음 ›** 버튼과 같은 동작이고,
둘 다 *현재 필터에 걸린* 목록만 훑습니다. 오른쪽 끝의 `3 / 16`이 그 안에서의 위치입니다.

에피소드가 100개를 넘어가면 사이드바의 **`flagged` 필터**로 warn/fail만 걸러
`J`/`K`로 넘기며 `G`/`B`로 판정하는 방식이 빠릅니다.
판정한 에피소드는 목록에서 초록/빨강/노랑으로 칠해지므로, 어디까지 봤는지 한눈에 들어옵니다.
...
**브라우저가 아니라 서버(로봇)의 파일에 저장됩니다.** `G`/`B`/`U`를 누르는 순간
서버로 전송되어 `episode_review.json`에 기록되므로:

- 새로고침해도 남습니다
- 다른 컴퓨터에서 눌러도 같은 파일에 쌓입니다
- 서버를 재시작해도 시작 시 파일을 다시 읽어 복원됩니다

여러 사람이 동시에 검수해도 파일이 깨지지 않도록, 저장 전체를 락으로 직렬화하고
쓰기마다 고유한 임시 파일에 쓴 뒤 원자적으로 교체합니다.

다만 **실시간 동기화는 없습니다.** 두 사람이 페이지를 동시에 열어둔 경우, 상대가
방금 누른 판정은 새로고침해야 보입니다. 같은 에피소드를 둘이 다르게 찍으면
나중에 누른 쪽이 남습니다. 에피소드를 나눠서 검수하면 문제되지 않습니다.
